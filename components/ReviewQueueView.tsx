...
import { RedactionEvent } from '../lib/types';
import { useAudioMeeting } from '../context/AudioMeetingContext';

interface ReviewQueueViewProps {
  events?: RedactionEvent[];
  onUpdateEventStatus?: (eventId: string, status: 'confirmed_true_positive' | 'marked_false_positive') => void;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3.5 py-1.5 font-medium text-amber-300">
            Pending: <strong className="font-mono">{pendingCount}</strong>
          </span>
          <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3.5 py-1.5 font-medium text-emerald-300">
            Confirmed: <strong className="font-mono">{confirmedCount}</strong>
          </span>
          <span className="rounded-full border border-rose-500/30 bg-rose-500/10 px-3.5 py-1.5 font-medium text-rose-300">
      ...
      <div className="rounded-2xl border border-emerald-500/30 bg-[#07080a] p-4 text-xs text-emerald-300 flex items-start gap-2.5 shadow-sm">
        ...
        <div>
          ...
          In strict compliance with architectural mandates, raw secrets are never shown to human reviewers. Only anonymized metadata, rule IDs, and safe masked context windows are accessible.
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#07080a] shadow-sm overflow-hidden">
        {filteredEvents.length === 0 ? (
          <div className="text-center py-14 text-white/40 text-xs">
            ...
            <p className="font-semibold text-white/70">No Review Events Match Filter</p>
            <p className="text-white/40 mt-0.5">Run a live meeting or simulation to generate new redaction events.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/[0.02] text-white/50 font-mono text-[11px]">
                  ...
                  <th className="py-3.5 px-5">RULE & LAYER</th>
                  <th className="py-3.5 px-5">CONFIDENCE</th>
                  <th className="py-3.5 px-5">SAFE MASKED CONTEXT</th>
                  <th className="py-3.5 px-5">STATUS</th>
                  <th className="py-3.5 px-5 text-right">FEEDBACK ACTIONS</th>
                </tr>
              </thead>
                ...
                {filteredEvents.map((evt) => (
                  <tr key={evt.id} className="hover:bg-white/[0.03] transition-colors">
                    ...
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      <div className="font-mono text-white text-[11px]">{evt.id.slice(0, 14)}...</div>
                    ...
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      ...
                      <div className="text-[10px] text-white/40 uppercase">{evt.severity}</div>
                    </td>

                    {/* Safe Masked Context */}
                    <td className="py-3.5 px-5 max-w-xs md:max-w-md truncate font-mono text-white/90 text-[11px]">
                    ...
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      {evt.status === 'pending_review' && (
                        <span className="rounded-full bg-amber-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-amber-300 border border-amber-500/30">
                          Pending
                        </span>
                      )}
                      {evt.status === 'confirmed_true_positive' && (
                        <span className="rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-emerald-300 border border-emerald-500/30">
                          Confirmed TP
                        </span>
                      )}
                      {evt.status === 'marked_false_positive' && (
                        <span className="rounded-full bg-rose-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-rose-300 border border-rose-500/30">
                          False Positive
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-5 whitespace-nowrap text-right space-x-1.5">
                      <button
                        ...
                        className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 px-2.5 py-1 text-[11px] font-medium text-emerald-300 transition-colors inline-flex items-center gap-1"
                      ...
                      <button
                        ...
                        className="rounded-lg border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/20 px-2.5 py-1 text-[11px] font-medium text-rose-300 transition-colors inline-flex items-center gap-1"
                      ...
                      <button
                        ...
                        className="rounded-lg border border-white/10 bg-white/[0.05] hover:bg-white/10 px-2.5 py-1 text-[11px] font-medium text-white transition-colors inline-flex items-center gap-1"
